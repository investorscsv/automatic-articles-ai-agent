...
def sync_image_url_to_article(article_dir: Path, image_path: str) -> None:
    """Write the approved image path into article metadata files."""
    meta_path = article_dir / "article_meta.json"
    record_path = article_dir / "article_record.json"
    if meta_path.exists():
        meta = json.loads(meta_path.read_text(encoding="utf-8"))
        meta["img_url"] = image_path
        meta_path.write_text(json.dumps(meta, ensure_ascii=False, indent=2), encoding="utf-8")
    if record_path.exists():
        record = json.loads(record_path.read_text(encoding="utf-8"))
        record["img_url"] = image_path
...
def review_image(state: WorkflowState) -> None:
    ...
    approved = False
    while not approved:
        generation_result = generate_cover_background({"topic_data": state.approved_topic}, image_dir / f"raw_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}")
        ...
        print("\nImage review:")
        print(f"Final cover file: {final_cover['output_image']}")
        print("Open that file locally to review the image. Most IDE terminals cannot render images inline.")
        choice = prompt_choice(
        ...
        sync_image_url_to_article(article_dir, final_cover["output_image"])
        state.status = "image_approved"
        state.publish = None
        save_state(state)
        print("The image has been approved.\n")
...
def ask_publish_decision(state: WorkflowState) -> None:
    ...
    print(f"- Final cover: {state.image['final_cover']['output_image']}")
    if prompt_yes_no("Do you want to publish this article now?", default="no"):
        publish_result = publish_article(article_dir)
